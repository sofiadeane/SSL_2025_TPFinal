...
#define NUMESTADOS 15       // cantidad total de estados que tiene el Autómata Finito (AF)
#define NUMCOLS 13          // cantidad total de clases de caracteres
#define TAMLEX 32+1         // longitud máxima de identificadores/lexemas
#define TAMNOM 20+1         // longitud máxima de nombres de archivo
...
typedef struct
{
char identifi[TAMLEX];  // Almacena el nombre del token
TOKEN t;                // Tipo de token (Palabra Reservada o ID)
} RegTS;
...
TOKEN clase;
char nombre[TAMLEX];
int valor; // Valor numérico para constantes
} REG_EXPRESION;
...
void Sentencia(void)
{
    ...
    switch ( tok )
    {
        case ID : /* -> ID :=  #asignar ; (rutina semantica)*/
            Identificador(&izq);
            Match(ASIGNACION);
            Expresion(&der);
            Asignar(izq, der);
...
void ListaExpresiones(void)
{
    /* ->  #escribir_exp {COMA  #escribir_exp} */
    TOKEN t;
    REG_EXPRESION reg;
    Expresion(&reg);
    Escribir(reg);
...
void Primaria(REG_EXPRESION * presul)
{
    ...
    switch ( tok )
    {
        case ID : /* ->  */
            ...
            break;
        case CONSTANTE : /* -> CONSTANTE #procesar_cte */
            Match(CONSTANTE);
            *presul = ProcesarCte();
            break;
        case PARENIZQUIERDO : /* -> PARENIZQUIERDO PARENDERECHO */
            Match(PARENIZQUIERDO);
            Expresion(presul);
            Match(PARENDERECHO);
            break;
